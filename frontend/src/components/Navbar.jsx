...
export default function Navbar({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white border-b border-slate-200/80 pt-[env(safe-area-inset-top)]">
            ...
            <>
              {user.role === 'student' && onToggleNotifications && (
                ...
                >
                  ...
                  {unreadNotificationsCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 min-w-4 h-4 px-1 rounded-full bg-slate-900 text-white font-semibold text-[9px] flex items-center justify-center">
              ...
              <button
                type="button"
                onClick={onLogout}
                className="h-9 px-2.5 sm:px-3 rounded-lg text-sm text-slate-600 hover:bg-slate-100 inline-flex items-center gap-1.5"
                title="Sign out"
              >
